...
export const BOOST_TIERS = [
  ...
  },
  {
    ...
    unlocks: "Свои эмодзи сервера и вложения до 100 МБ",
  },
] as const;

/** Сколько своих эмодзи можно держать на сервере третьего уровня.
 *
 *  Пятьдесят — с запасом: у компании из четверых их набирается
 *  десяток-другой, а каждое лишнее едет картинкой в каждое сообщение,
 *  где стоит. Предел нужен не ради диска, а чтобы список оставался
 *  списком, по которому можно найти нужное глазами. */
export const EMOJI_LIMIT = 50;

/** Правило имени: латиница, цифры и подчёркивание.
 *
 *  Имя стоит в тексте между двоеточиями, и всё, что можно спутать
 *  с обычной речью, отсюда исключено: пробел разорвал бы имя надвое,
 *  а кириллица в `:привет:` спорила бы с обычным смайликом из
 *  двоеточия и скобки. */
