...
export interface Preferences {
  ...
  mutedUsers: string[];
  /**
   * Громкость чужого показа отдельно от его же голоса: кто → 0–5.
   *
   * Это разные звуки с разными задачами. Игра у показывающего может
   * реветь так, что его самого не слышно, — и выбор был скверный:
   * либо терпеть, либо приглушить человека вместе с его голосом,
   * то есть перестать слышать то, ради чего в разговоре и сидят.
   */
  screenGain: Record<string, number>;
...
const DEFAULTS: Preferences = {
  ...
  mutedUsers: [],
  screenGain: {},
  // По умолчанию 1080p и 30 кадров.
  //
  // Было 720p — из осторожности к чужому компьютеру: захват в родном
  // разрешении дорого обходится тем, у кого он идёт медленным путём.
  // Осторожность оказалась не в том месте. Замер (check-share.cjs:
  // та же картинка в три соединения сразу) показал, что дело не
  // в захвате, а в кодеке: браузер по умолчанию берёт VP8, жмёт его
  // процессор, и при скупой полосе он режет кадр до 640×360 и выдаёт
  // пятнадцать кадров в секунду. Это и видели как «плохое качество
  // и фпс маленький».
  //
  // Теперь показ идёт через H264 — его жмёт видеокарта, — и 1080p
  // при тридцати кадрах держится в родном размере, ни во что
