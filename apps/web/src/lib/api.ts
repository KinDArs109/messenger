...
/**
 * Токен, которым точно можно пользоваться прямо сейчас.
 *
 * Access-токен живёт пятнадцать минут и обновляется по ходу дела:
 * запрос получил 401 — обновились, повторили. Пока человек что-то
 * делает, этого хватает.
 *
 * А вот когда мессенджер свёрнут, ничего не происходит: запросов нет,
 * обновлять нечего, токен тихо протухает. Сокет в это время живёт
 * своей жизнью — и как только связь моргнёт, он переподключается
 * с тем самым протухшим токеном, получает от ворот поворот и стучится
 * так до бесконечности. Снаружи это выглядит как «свернул на час,
 * развернул — а связи нет».
 *
 * Поэтому перед каждым подключением спрашиваем здесь: если до конца
 * жизни токена осталась минута или он уже истёк — сначала обновляем.
 */
export async function freshAccessToken(): Promise<string | null> {
  if (accessToken && !скороИстечёт(accessToken)) return accessToken;
  await refreshOnce();
  return accessToken;
}

/** Сколько токену осталось. Подпись не проверяем — это дело сервера;
 *  нам нужно только время, а оно лежит в открытой части. */
function скороИстечёт(token: string): boolean {
  try {
    const часть = token.split(".")[1];
    if (!часть) return true;
    const данные = JSON.parse(atob(часть.replace(/-/g, "+").replace(/_/g, "/"))) as {
      exp?: number;
    };
    if (typeof данные.exp !== "number") return true;
    return данные.exp * 1000 - Date.now() < 60_000;
  } catch {
    // Не разобрали — считаем, что пора обновить: обновление дешевле
    // молчаливого разрыва.
    return true;
  }
}
