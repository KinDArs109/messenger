...
let дверьДо = null; // до какого времени она открыта
let данные = null;
let вСети = new Set();
 ...
 * Живое обновление.
 *
 * Панель показывала снимок на момент входа. Удалил сервер — он исчезал,
 * потому что после удаления панель перечитывала всё сама; а вот кто-то
 * зашёл, кто-то вышел, кого-то удалили из мессенджера — этого она
 * не узнавала никогда, и «сейчас в сети» врало тем увереннее, чем
 * дольше окно оставалось открытым.
 *
 * Теперь она перечитывает хозяйство каждые несколько секунд и
 * перерисовывает, только если что-то и правда изменилось: иначе
 * список дёргался бы и терял место прокрутки на ровном месте.
 */
const ЖИВОЕ_МС = 8000;
let часы = null;
let слепок = "";
let сказано = null; // { текст, до } — успех показываем недолго
let вСвязи = true;
...
function показатьВход(ошибка) {
  // Часы живого обновления на экране входа не нужны — и не должны
  // стучаться на сервер с мёртвым токеном.
  перестать();
  корень.innerHTML = `
    <div class="вход">
...
/** Открыть хозяйство и дальше держать его живым. */
async function открыть(сообщение) {
  if (сообщение) сказано = { текст: сообщение, до: Date.now() + 12_000 };

  try {
    await перечитать(true);
  } catch (беда) {
    перестать();
    // Не хозяин — раздела для него просто нет. Но ровно так же
    // выглядит и закрывшаяся дверь, а это совсем другое дело.
    показатьВход(
...
/**
 * Перечитать хозяйство.
 *
 * Перерисовываем, только если что-то изменилось: панель — это одна
 * большая перерисовка, и делать её каждые восемь секунд впустую значит
 * гонять место прокрутки туда-сюда у человека под рукой.
 */
async function перечитать(силой) {
  const свежее = await запрос("/admin/overview");

  let сеть = вСети;
  try {
    сеть = new Set((await запрос("/admin/online")).вСети ?? []);
  } catch {
    // Кто в сети — не то, ради чего стоит показывать ошибку.
  }

  const слепок2 = JSON.stringify([свежее, [...сеть].sort()]);
  const естьЧтоСказать = Boolean(сказано);
  данные = свежее;
  вСети = сеть;

  if (!силой && слепок2 === слепок && !естьЧтоСказать) return;
  слепок = слепок2;
  нарисовать();
}

/** Продлить вход, пока дверь открыта. Токен живёт пятнадцать минут,
 *  а панель — сколько на неё смотрят. */
async function продлить() {
  if (!скороИстечёт(токен)) return;
  const ответ = await запрос("/admin/session/renew", { method: "POST" });
  токен = ответ.accessToken;
  if (ответ.дверьДо) дверьДо = ответ.дверьДо;
}

/** Осталось ли токену жить. Подпись не наше дело — нам нужен только
 *  срок, а он лежит в открытой части. */
function скороИстечёт(токен) {
  try {
    const часть = токен.split(".")[1];
    const { exp } = JSON.parse(
      atob(часть.replace(/-/g, "+").replace(/_/g, "/")),
    );
...
/** Один такт живого обновления. */
async function поддержать() {
  // Спрятанное окно не трогаем: смотреть в него некому, а сервер
  // дёргать незачем.
  if (!дверь || document.hidden) return;

  try {
    await продлить();
    await перечитать(false);
    if (!вСвязи) {
      вСвязи = true;
      нарисовать();
    }
  } catch (беда) {
    // Сети нет — это не повод разбирать панель: покажем это в шапке
    // и попробуем через восемь секунд.
    if (беда instanceof TypeError) {
      if (вСвязи) {
        вСвязи = false;
        нарисовать();
      }
    ...
    перестать();
    дверь = null;
    // Дверь открыта полчаса и закрывается сама. Снаружи это выглядит
    // как «раздел не найден», и объяснять это надо словами, а не
    // выкидывать человека на ввод пароля.
    if (беда.message.includes("не найден")) {
      await спроситьТелеграм("Полчаса прошло, дверь закрылась. Вот новый код.");
    } else {
      показатьВход(беда.message);
    }
  }
...
function следить() {
  перестать();
  часы = setInterval(() => void поддержать(), ЖИВОЕ_МС);
  document.addEventListener("visibilitychange", приВозвращении);
  window.addEventListener("focus", приВозвращении);
}

function перестать() {
  clearInterval(часы);
  часы = null;
  document.removeEventListener("visibilitychange", приВозвращении);
  window.removeEventListener("focus", приВозвращении);
}

/** Вернулись к окну — самый нужный момент перечитать: как раз тогда,
 *  когда панель провисела свёрнутой полчаса. */
function приВозвращении() {
  if (!document.hidden) void поддержать();
}
...
/** Час и минуты — для срока двери. Секунды тут только шумят. */
const время = (мс) =>
  new Date(мс).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" });
...
function нарисовать() {
  ...
  // Сказанное живёт недолго: «сервер удалён» через минуту уже не новость,
  // а панель к этому времени перерисовалась не раз.
  if (сказано && сказано.до < Date.now()) сказано = null;
  const сообщение = сказано?.текст;

  // Место прокрутки — наше дело: панель перерисовывается целиком,
  // и без этого живое обновление подбрасывало бы страницу наверх
  // ровно тогда, когда в неё смотрят.
  ...
  корень.innerHTML = `
    <header>
      ...
      <span class="справа">
        ...
        <span class="значок ${вСвязи ? "да" : "нет"}" id="живое"
              title="${вСвязи ? "Панель перечитывает хозяйство сама, каждые несколько секунд. Нажмите, чтобы перечитать сейчас" : "Сервер не отвечает — попробуем снова через несколько секунд"}">
          ${вСвязи ? "живое" : "нет связи"}
        </span>
        ${дверьДо ? `<span class="значок нет" title="Дверь открыта на полчаса. Потом — новый код в Телеграме">дверь до ${время(дверьДо)}</span>` : ""}
...
function повеситьРучки() {
  // Значок живого обновления — заодно и кнопка «сейчас»: ждать восемь
  // секунд, когда точно знаешь, что там изменилось, незачем.
  document.getElementById("живое").onclick = () => void поддержать();
