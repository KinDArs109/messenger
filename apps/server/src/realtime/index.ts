...
/** Кто сейчас на связи — по живым сокетам, а не по последнему входу.
 *  Нужно хозяйской панели: «заходил вчера» и «сидит прямо сейчас» —
 *  разные вещи. */
export function ктоВСети(): string[] {
  return [...connections.entries()].filter(([, сколько]) => сколько > 0).map(([id]) => id);
}
