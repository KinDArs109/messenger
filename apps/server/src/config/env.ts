...
const envSchema = z.object({
  ...
  // Раздача картинки: сервер принимает показ один раз и рассылает
  // остальным (см. realtime/sfu.ts).
  //
  // Ничего задавать не нужно — раздача поднимается сама, если
  // библиотека для неё собрана под эту систему. На машине
  // разработчика её обычно нет, и это не беда: мессенджер вернётся
  // к прежнему способу «каждому свой поток».
  /** Выключить раздачу принудительно — например, чтобы сравнить
   *  с прежним способом или если на маленькой машине не хватает сил. */
  SFU_DISABLED: z.coerce.boolean().default(false),
  /** Каким адресом сервер виден снаружи. Пусто — берётся TURN_HOST,
   *  а если и его нет, раздача объявит адрес, по которому её нашли. */
  SFU_ANNOUNCED_IP: blank(z.string()),
  /** Порты для медиа. Их надо открыть в файрволе: на каждого человека
   *  уходит по одному на дорогу, а дорог две. Полсотни хватает
   *  с запасом на компанию друзей. */
  SFU_MIN_PORT: z.coerce.number().int().positive().default(40000),
  SFU_MAX_PORT: z.coerce.number().int().positive().default(40059),
