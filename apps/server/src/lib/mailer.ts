...
/** Письмо с кодом на вход.
 *
 *  Текст свой, не общий с подтверждением почты, и это важнее, чем
 *  кажется. Такое письмо — единственное, что увидит человек, чей
 *  пароль узнали: код пришёл, а входа не было. Поэтому в нём прямо
 *  написано, что делать, и написано первым делом. */
export async function sendLoginCode(to: string, code: string): Promise<boolean> {
  return send(
    to,
    `Вход в мессенджер: ${code}`,
    [
      `Код для входа: ${code}`,
      "",
      "Он действует 15 минут.",
      "Если вы не входили — кто-то знает ваш пароль.",
      "Смените его: в мессенджере «Забыли пароль?» на форме входа.",
    ].join("\n"),
    `
      <div style="font-family:system-ui,Segoe UI,sans-serif;max-width:420px">
        <p style="color:#4b5563">Код для входа:</p>
        <p style="font-size:32px;letter-spacing:6px;font-weight:700;margin:12px 0">${code}</p>
        <p style="color:#6b7280;font-size:14px">Он действует 15 минут.</p>
        <p style="color:#9ca3af;font-size:13px">
          Если вы не входили — кто-то знает ваш пароль. Смените его:
          на форме входа есть «Забыли пароль?».
        </p>
      </div>`,
  );
}
