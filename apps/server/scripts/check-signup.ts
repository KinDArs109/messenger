...
async function main() {
  ...
  const первый = await зарегистрировать("otkryt", undefined);

  // Ограничитель считает неудачные попытки по адресу и помнит их
  // четверть часа. Эта проверка их и устраивает — семь штук за прогон, —
  // поэтому два прогона подряд упираются в него на первом же шаге.
  // Сказать об этом надо прямо: молча выдать «всё хорошо» хуже всего.
  if (первый.status === 429) {
    ok("ограничитель отпустил прошлый прогон", false, {
      подсказка: "подождите четверть часа или перезапустите сервер",
    });
    return;
  }

  const открыт = первый.status === 201;
  if (открыт) {
  ...
  // Ни писем, ни заставы: человек попадает в переписку сразу.
  // Раньше здесь стоял отказ «подтвердите почту» — от него отказались,
  // потому что он встречал позванного человека вместо мессенджера.
  ok("и человек сразу внутри — подтверждать ничего не надо", сразу === 200, { статус: сразу });
  ...
  // Подтвердить адрес всё ещё можно — по своей воле.
